...
# EXECUTION
files = [("./data/lynx_error_predict.xlsx", -14),
        ("./data/mkglass_error_predict.xlsx", -500),
        ("./data/sunspot_error_predict.xlsx", -67),
        ("./data/txcambio_error_predict.xlsx", -52)]
...
for f in files:
    dataset, testdataset = load_data(f[0], index_test=f[1], lag_max=lag_max, split_ratio=split_ratio)
    print("Carregando arquivo: {}".format(f[0]))
    best_score = np.inf
    ...
    for g in range(nGens):
        ...
        with open(filename, 'a', newline="") as csvfile:
            ...
            for i, d in enumerate(zip(gen.get_pop(), gen.get_error_list())):
                writer.writerow([f[0], g+1, i+1, d[0], d[1]])
